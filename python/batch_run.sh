...
do
	...
	for target_obj in Dog_1 Dog_2 Dog_3 Dog_4 Dog_5
	do
		python run.py $target_obj --corr_reg=${pp} --multiscale --logdir=${logdir} 2>&1 | tee ${logdir}/log_${target_obj}.txt
	done
	for target_obj in Patient_1 Patient_2
	do
		python run.py $target_obj --downsample=10 --corr_reg=${pp} --multiscale --logdir=${logdir} 2>&1 | tee ${logdir}/log_${target_obj}.txt
	done
done
